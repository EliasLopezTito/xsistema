    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (datos.respuesta == "success") {
                console.log("x"+datos.arrayProgramaciones[0].codLocal)
                programacionRegNotas(anioProg,mesProg,datos.arrayProgramaciones[0].codLocal)
                ...
                programacionRegNotas(anioProg,mesProg,null)
                var errores = "";
...
function programacionRegNotas(anioProg, mesProg, cod_local) {
    
    console.log("xd", cod_local);
    if (cod_local != null && cod_local != "vacio") {
        $.ajax({
            url: path + "supervisor/controlDocentes",
            type: "POST",
            data: {
                anio: anioProg,
                mes: mesProg,
                cod_local: cod_local,
                opcion: "Programaciones"
            },
            success: function (data) {
                var datos = JSON.parse(data);
                document.getElementById("fechaInicio1").textContent = datos.data[0].FInicio;
                document.getElementById("fechaTermino1").textContent = datos.data[0].FTerminto;
                document.getElementById("fechaInicio2").textContent = datos.data[0].FInicio2;
                document.getElementById("fechaTermino2").textContent = datos.data[0].FTerminto2;
            }
        });
    } else {
        document.getElementById("fechaInicio1").textContent = "";
        document.getElementById("fechaTermino1").textContent = "";
        document.getElementById("fechaInicio2").textContent = "";
        document.getElementById("fechaTermino2").textContent = "";
    }
}