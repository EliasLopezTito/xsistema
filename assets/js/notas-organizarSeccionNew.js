...
function cargarSedesSeleccionadas(){
    setTimeout(() => {
         $.ajax({
            url: path + "notas/organizarSeccionNew",
            type: "POST",
            dataType: "JSON",
            data: {
                codLocal: $('#institucion').val(),  
                codTipoEspe: $('#tipoEspecialidad').val(),  
                codEspe: $('#especialidad').val(),            
                opcion: "cargarSedesSeleccion"
            },   
            beforeSend: function(){
                $("#sede").html("");
            },  
                ...
                if ( response.respuesta === "success" ) {
                    ...
                    if ( response.data != "vacio" ) {
                        response.data.forEach( (e,key) => {
                            $("#sede").append(`
                                <option value="${e.cod_localinst.trim()}">${e.cod_localinst.trim()} - ${e.descripcion_local.trim()}</option>
                            `)
                        });
                    } else {
                        $("#sede").append(`
