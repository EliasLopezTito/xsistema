            ...
            if( res.respuesta === "success" ){
                ...
                if(res.cursos.length > 0){
                    ...
                    res.cursos.forEach( ( val , key ) => {                      
                        const tr = `
                            <tr class="tr-cursos" cod_local="${val.local.trim()}" tipoespe="${val.tipo.trim()}" cod_espe="${val.espe.trim()}" cod_localinst="${val.local.trim()}" hora="${val.hora.trim()}" cod_ciclo="${val.Ciclo.trim()}" cod_curso="${val.cod_curso.trim()}"  cod_aula="${val.cod_aula.trim()}" mes="${val.mes.trim()}" turno="${val.Turno.trim()}" anio="${val.ano}" >
                ...
                if(res.pagos.length > 0){
                    res.pagos.forEach( ( val , key ) => {                      
                        const tr = `
                            <tr>
                                <td class="text-center" >${val.Transaccion.trim()}</td>
                                <td class="text-center" >${val.Fecha.trim()}</td>
