    ...
    $("#docente").autocomplete({
        ...
        select: function(event, ui){
            ...
            if (ui.item.vacio) {
                ...
                mostrarDatos(ui.item.Telefonos, ui.item.Correo , ui.item.cod_emp , ui.item.cod_local );
            }
            return false;
        }
    })
...
function mostrarDatos(tel, correo , cod_emp , cod_local ) {
    $('#div-datos').show('300');
    $('#lbl-cod-docente').text(cod_emp);
    $('#lbl-cod-local').text(cod_local);
...
function verProgramaciones(){
    ...
    $.ajax({
        url: path + "centroCapacitacion/controlDocentes",
        type: "POST",
        data: {
        	anioProg: anioProg,
        	mesProg: mesProg,
            ...
            if (datos.respuesta == "success") {
                ...
                if (datos.arrayNotasProgramacion.length > 0){
                    ...
                    datos.arrayNotasProgramacion.forEach( ( alumno , key ) => {
                        ...
                        const tr = `<tr>
                                <input type="hidden" name="tipo_grabado[]" value="${alumno.tipo_grabado}"> 
                                <input type="hidden" name="id_NotaD[]" value="${alumno.id_NotaDocente}"> 
                                <input type="hidden" name="cod_alumno[]" value="${alumno.cod_alumno}">
                                ...
                                <td class="celda-centrada">
                                    <input readonly type="text" class="form-control notas_promedio" onblur="validarNota(this)" style="text-align: center; padding-left: 0px; padding-right: 0px;text-transform: uppercase;" name="ap[]" value="${alumno.ap.trim()}" />
                                </td>

                                <td class="celda-centrada" style="display:none">
                                    <input readonly type="text" class="form-control notas_promedio" onblur="validarNota(this)" style="text-align: center; padding-left: 0px; padding-right: 0px;text-transform: uppercase" name="ec[]" value="${alumno.ec.trim()}" />
                                </td>

                                <td class="celda-centrada">
                                    <input readonly type="text" class="form-control notas_promedio" onblur="validarNota(this)" style="text-align: center; padding-left: 0px; padding-right: 0px;text-transform: uppercase" name="ac[]" value="${alumno.ac.trim()}" />
                                </td>

                                <td class="celda-centrada">
                                    <input readonly type="text" class="form-control notas_promedio" onblur="validarNota(this)" style="text-align: center; padding-left: 0px; padding-right: 0px;text-transform: uppercase" name="ef[]" value="${alumno.ef.trim()}" />
                                </td>

                                <td class="celda-centrada">
                                    <input readonly type="text" class="form-control" onblur="validarNota(this)" style="text-align: center; padding-left: 0px; padding-right: 0px;text-transform: uppercase" name="pr[]" value="${alumno.pr.trim()}" />
                                </td>                                                                    
                            </tr>`;
                        tbody.append(tr);
...
function validarNota(text) {
    ...
    let nota = $(text).val().trim();
    $(text).css({"color": "black"});

    if ( isNaN(nota) ) {
        $(text).val("")//.css({"color": "red"});
    } else if( nota === "" ){
        $(text).val("")//.css({"color": "red"});
    } else {
