...
$('#form-diplomas').submit(function (e) {
    e.preventDefault();
    ...
    tablaTitulado = $("#tablaListado").DataTable({
        ...
        columns: [
            ...
            },
            {
                data: null,
                render: function (data) {
                    return data.Nro_Bachiller
                }
            },
            {
                data: null,
                render: function (data) {
                    return data.Nro_Oficio
                }
            },
            {
                data: null,
                render: function (data) {
                    return data.FechaMinedu
                }
            },
            {
                data: null,
                render: function (data) {
                    return data.FechaIngreso;
                }
            },
            {
                data: null,
                render: function (data) {
                    ...
                    return "<button class=\"btn boton-tabla boton-naranja\" type=\"button\" onclick=\"editarDiplomas("+data.Cod_Bachiller+");\" title=\"Editar Trámite\"><span class=\"icon-pencil\"></span></button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + 
                            "<button class=\"btn boton-tabla boton-rojo\" type=\"button\" onclick=\"eliminarDiplomas("+data.Cod_Bachiller+");\" title=\"Eliminar Trámite\"><span class=\"icon-cross\"></span></button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
...
function editarDiplomas(Cod_Titulado) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (datos.respuesta == "success") {
                ...
                //$("#alumno").val(alumno);
                $("#sede").val(datex.sede.trim());
                $("#institucion").val(datex.Cod_local);
                $("#tipoEspecialidad").val(datex.Tipo_espe);
                $("#especialidad").val(datex.Cod_espe);
                $("#Nro_Bachiller").val(datex.Nro_Bachiller.trim());
                $("#Nro_Oficio").val(datex.Nro_Oficio.trim());
                $("#FechaMinedu").val(datex.FechaMinedu.substr(0, 10));
                $("#FechaIngresoExp").val(datex.FechaIngresoExp.substr(0, 10));
    ...
    //$("#alumno").focus();
    $("#tituloModal").html("Editar Diploma");
    $("#modalNuevoTramite").modal({ backdrop: 'static', keyboard: false });

}

function eliminarDiplomas(Cod_Titulado) {
    ...
    Notiflix.Confirm.Show(
        'Eliminar Diploma',
        '¿Esta segura de eliminar la diploma?',
        'Si',
        'No',
        function okCb() {
