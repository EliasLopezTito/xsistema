    ...
    tablaEgresados = $("#tablaListaFaltaDocentes").DataTable({
        ...
        ajax: {
            ...
            dataSrc: function(response){ 
                ...
                $(".totalDocentes").text("TOTAL DOCENTES EN EL MES " + mesFecha + " DEL " + anioProg + " : ");
                $("#cantidadDocen").text(response.arrayDocente[0].NroDocente);
        ...
        columns: [
            ...
            { data: null, render: function (data) { return data.Curso; } },
            {
                ...
                render: function (data) {                 
                    return `
                        <button class='btn boton-tabla btn-success' type='button' 
                        onclick="imprimirNotasByProgramacion('${data.CodDocente.trim()}',
                        '${data.Cod_local.trim()}',
                        '${data.Tipo_espe.trim()}',
                        '${data.Cod_espe.trim()}',
                        '${data.Ciclo.trim()}','${data.semestre.trim()}',
                        '${data.CodMallaCurricular.trim()}',
                        '${data.CodCurso.trim()}',
                        '${data.Aula.trim() }',
                        '${data.turno.trim()}')"><span class='icon-printer'></span></button>
                    `; 
